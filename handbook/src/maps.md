...
>
> Async maps can use the [async channel] to access buffer data, but each query needs to wait until the next execution flush takes place before it will get a reply.
...
To get access to these, you must define your function explicitly with [`Blocking`] or [`Async`]:

```rust,no_run,noplayground
{{#include ./examples/handbook_snippets/src/native-snippets.rs:fibonacci_map_example}}
```

Before passing your function into `commands.request(_)` you must apply `.into_map()` to it to convert it into a map.
...
Like when accessing streams, you just need to make it explicit that you have an async map:

```rust,no_run,noplayground
{{#include ./examples/handbook_snippets/src/native-snippets.rs:navigate_map_example}}
```

With that, `navigate` can be passed into `commands.request(_, navigate.into_map())` or into `series.map(navigate)` or `chain.map(navigate)`.
