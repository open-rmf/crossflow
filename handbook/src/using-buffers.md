...
{{#include ./examples/native/src/handbook_snippets.rs:buffer_settings_keep_all}}
```

The data from the buffers will be joined into `LocalizationData`, defined here:

```rust,no_run,noplayground
{{#include ./examples/native/src/handbook_snippets.rs:LocalizationData}}
```
...
```rust,no_run,noplayground
{{#include ./examples/native/src/handbook_snippets.rs:join_settings_clone}}
```
